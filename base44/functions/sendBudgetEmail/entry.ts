...
<body>
  <div style="max-width:700px; margin:0 auto; padding:20px;">
    ...
    <div style="background:#fff; border:1px solid #e5e7eb; border-top:none; padding:24px 32px;">
      ...
      <!-- Custos adicionais -->
      ${(budget.travel_cost || budget.fuel_cost || (budget.additional_fees && budget.additional_fees.length > 0) || budget.discount_percentage) ? `
      <div style="border-top:2px solid #e5e7eb; padding-top:20px; margin-top:8px; margin-bottom:20px;">
        <table style="width:100%; font-size:14px;">
          ${budget.travel_cost > 0 ? `<tr><td style="padding:8px 0; color:#6b7280;">Deslocamento</td><td style="text-align:right; font-weight:600; color:#111827;">R$ ${fmt(budget.travel_cost)}</td></tr>` : ''}
          ${budget.fuel_cost > 0 ? `<tr><td style="padding:8px 0; color:#6b7280;">Combustível</td><td style="text-align:right; font-weight:600; color:#111827;">R$ ${fmt(budget.fuel_cost)}</td></tr>` : ''}
