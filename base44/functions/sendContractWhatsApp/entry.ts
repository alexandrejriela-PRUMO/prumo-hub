...
async function logWhatsAppSend(base44, data) {
  try {
    ...
    console.error('[sendContractWhatsApp] Erro ao gravar WhatsAppSendLog:', e.message);
  }
}

Deno.serve(async (req) => {
  let base44, contract_id, phone, contract, sendMessage, sendFileName;
  try {
    ...
    ({ contract_id, phone, pdf_url, file_name, message } = await req.json());
    ...
    contract = await base44.entities.ClientContract.get(contract_id);
    ...
    sendFileName = file_name || `Contrato_${contract.client_name || contract_id}.pdf`;
