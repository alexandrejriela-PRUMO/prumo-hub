...
/**
 * updateClientContract — Atualiza um registro de ClientContract via service role.
 *
...
Deno.serve(async (req) => {
  try {
    ...
    const updated = await base44.asServiceRole.entities.ClientContract.update(id, data);
    return Response.json(updated);
  } catch (error) {
    console.error('[updateClientContract] Erro:', error.message);
