...
Deno.serve(async (req) => {
  if (req.method === 'GET') {
    return Response.json({ status: 'ok', webhook: 'webhookTransacaoEstornada' }, { status: 200 });
  }
  ...
  // const expectedToken = Deno.env.get('WEBHOOK_TOKEN_ESTORNADO');
  ...
  console.log('[webhookTransacaoEstornada] HEADERS:', JSON.stringify(Object.fromEntries(req.headers.entries()), null, 2));
  console.log('[webhookTransacaoEstornada] PAYLOAD:', JSON.stringify(body, null, 2));

  const { email, fullName } = extractBuyer(body);

  if (!email) {
    console.warn('[webhookTransacaoEstornada] Email não encontrado no payload.');
  ...
  try {
    ...
    if (!users || users.length === 0) {
      console.log(`[webhookTransacaoEstornada] Usuário não encontrado: ${email}`);
      return Response.json({ received: true, message: 'Usuário não encontrado.', email }, { status: 200 });
    }

    // BLOQUEAR usuário por estorno
    await base44.asServiceRole.entities.User.update(users[0].id, {
      status: 'inactive',
      blocked_reason: 'transacao_estornada',
      blocked_at: new Date().toISOString(),
    });

    console.log(`[webhookTransacaoEstornada] Usuário bloqueado por estorno: ${email}`);
    return Response.json({ received: true, message: 'Usuário bloqueado por estorno (reembolso).', email }, { status: 200 });
  } catch (error) {
    console.error('[webhookTransacaoEstornada] Erro:', error.message);
