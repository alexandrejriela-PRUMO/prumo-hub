  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {effectivePropertyId && (
          <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={() => setEditOpen(true)}>
            {carRecord ? <><Edit className="w-4 h-4 mr-2" />Editar CAR</> : <><Plus className="w-4 h-4 mr-2" />Cadastrar CAR</>}
          </Button>
        )}
      </div>
      ...
      {/* Sem CAR cadastrado */}
      {effectivePropertyId && !carLoading && !carRecord && (
        <Card className="border-dashed border-2 border-emerald-200">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-gray-500 mt-2 mb-6">Cadastre as informações do CAR desta propriedade</p>
            <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={() => setEditOpen(true)}>
              <Plus className="w-4 h-4 mr-2" />Cadastrar CAR
            </Button>
          </CardContent>
        </Card>
      )}
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{carRecord ? 'Editar CAR' : 'Cadastrar CAR'}</DialogTitle>
          </DialogHeader>
          <CARForm
            initial={carRecord || {}}
