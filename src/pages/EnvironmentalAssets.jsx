...
import {
  Leaf, Plus, Trash2, X, FileText, Upload,
  ArrowRightLeft, Info, CheckCircle2, Clock
} from 'lucide-react';
...
export default function EnvironmentalAssets() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const extra = JSON.stringify({
        biome: data.biome, state: data.state, municipality: data.municipality,
        seller_name: data.seller_name, seller_cpf_cnpj: data.seller_cpf_cnpj,
        seller_car: data.seller_car, seller_property_name: data.seller_property_name,
        seller_state: data.seller_state, seller_municipality: data.seller_municipality,
        seller_phone: data.seller_phone,
        buyer_name: data.buyer_name, buyer_cpf_cnpj: data.buyer_cpf_cnpj,
        buyer_property_name: data.buyer_property_name, buyer_state: data.buyer_state,
        buyer_municipality: data.buyer_municipality, buyer_phone: data.buyer_phone,
        issuing_body: data.issuing_body, certification_number: data.certification_number,
        certification_date: data.certification_date, registry_number: data.registry_number,
      });
      ...
      if (editingTx?.id) return base44.entities.CRATransaction.update(editingTx.id, payload);
    ...
    onSuccess: async () => {
      ...
      setEditingTx(null);
      setForm(EMPTY_FORM);
      toast.success(editingTx ? 'Transação atualizada!' : 'Transação registrada!');
    },
  ...
  const openEdit = (tx) => {
    ...
    setEditingTx(tx);
    setForm({ ...EMPTY_FORM, cra_number: tx.cra_number || '', area_hectares: tx.area_hectares || '',
      price_per_hectare: tx.price_per_hectare || '', transaction_date: tx.transaction_date || '',
      status: tx.status || 'Em negociação', seller_email: tx.seller_email || '',
      buyer_email: tx.buyer_email || '', buyer_car: tx.buyer_car || '',
      notes: tx.notes || '', documents: tx.documents || [], ...extra });
    setShowForm(true);
  };
  ...
  const closeForm = () => { setShowForm(false); setEditingTx(null); setForm(EMPTY_FORM); };
    ...
    <div className="w-full space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="transacoes" className="space-y-4 mt-4">
          {!showForm && (
            <Button onClick={() => { setForm(EMPTY_FORM); setEditingTx(null); setShowForm(true); }} className="bg-emerald-600 hover:bg-emerald-700">
            ...
            <Card className="border-2 border-emerald-300">
              <CardHeader className="border-b border-emerald-200 bg-emerald-50">
                <div className="flex justify-between items-center">
                  <div>
                    <CardTitle className="text-emerald-900">{editingTx ? 'Editar Transação' : 'Nova Transação de Compra e Venda de CRA'}</CardTitle>
                    <CardDescription>Preencha os dados conforme o instrumento jurídico (Lei 12.651/2012)</CardDescription>
                  </div>
                  <Button size="sm" variant="ghost" onClick={closeForm}><X className="w-4 h-4" /></Button>
                </div>
              </CardHeader>
              <CardContent className="pt-6 space-y-8">
                    ...
                    {form.area_hectares && form.price_per_hectare && (
                      <div className="flex items-end">
                        <div className="w-full bg-emerald-100 border border-emerald-300 rounded-lg p-3">
                          ...
                          <p className="text-lg font-bold text-emerald-900">R$ {totalValue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {/* VENDEDOR */}
                <section className="space-y-4">
                  <h3 className="font-bold text-emerald-800 border-b border-emerald-100 pb-2">👤 Dados do Vendedor (Proprietário da CRA)</h3>
                  <div className="grid md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-1.5">
                      <Label>CAR da Propriedade Cedente</Label>
                ...
                {/* COMPRADOR */}
                <section className="space-y-4">
                  <h3 className="font-bold text-emerald-800 border-b border-emerald-100 pb-2">🏡 Dados do Comprador (Imóvel a ser Compensado)</h3>
                  <div className="grid md:grid-cols-3 gap-4">
                    <div className="space-y-1.5 md:col-span-2">
                      <Label>Nome / Razão Social *</Label>
                    ...
                    <div className="space-y-1.5">
                      <Label>Propriedade vinculada (sistema)</Label>
                ...
                {/* CERTIFICAÇÃO */}
                <section className="space-y-4">
                  <h3 className="font-bold text-emerald-800 border-b border-emerald-100 pb-2">🏛️ Certificação e Registro</h3>
                ...
                {/* DOCUMENTOS */}
                <section className="space-y-4">
                  <h3 className="font-bold text-emerald-800 border-b border-emerald-100 pb-2">📎 Documentos da Transação</h3>
                  <div className="flex flex-wrap gap-3">
                    {['Contrato de Cessão de CRA', 'Certificado CRA', 'CAR Validado (Cedente)', 'CAR Validado (Cessionário)', 'Matrícula do Imóvel', 'Documento de Identidade', 'Outro'].map(type => (
                      <FileUploadButton key={type} label={type} docType={type} onUploaded={addDoc} uploading={uploading} setUploading={setUploading} />
                    ))}
                  </div>
                ...
                <section className="space-y-2">
                  ...
                  <Textarea placeholder="Condicionantes, prazos, cláusulas específicas do contrato..." rows={3} value={form.notes} onChange={e => set('notes', e.target.value)} />
                </section>

                <div className="flex gap-3 justify-end pt-4 border-t border-emerald-100">
                  <Button variant="outline" onClick={closeForm}>Cancelar</Button>
                  <Button onClick={() => saveMutation.mutate(form)} className="bg-emerald-600 hover:bg-emerald-700 text-white" disabled={saveMutation.isPending}>
                  ...
                  <Card key={tx.id} className="hover:shadow-md transition-all">
                    <CardContent className="p-5 space-y-4">
                      <div className="flex justify-between items-start gap-4 flex-wrap">
                        <div className="space-y-1">
                          ...
                          {tx.price_per_hectare > 0 && (
                            <p className="text-sm text-gray-600">R$ {tx.price_per_hectare?.toLocaleString('pt-BR')}/ha · <strong>Total: R$ {tx.total_value?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</strong></p>
                          )}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Select value={tx.status} onValueChange={v => updateStatusMutation.mutate({ id: tx.id, status: v })}>
                            <SelectTrigger className="h-7 text-xs w-36 border-gray-300"><SelectValue /></SelectTrigger>
                            <SelectContent>
                              <SelectItem value="Em negociação">Em negociação</SelectItem>
                              <SelectItem value="Contrato assinado">Contrato assinado</SelectItem>
                              <SelectItem value="Concluída">Concluída</SelectItem>
                      ...
                      {(extra.issuing_body || extra.certification_number || extra.registry_number) && (
                        <div className="bg-amber-50 rounded-lg p-3 border border-amber-100 text-xs text-amber-800 flex flex-wrap gap-3">
                          <span className="font-semibold text-amber-700">CERTIFICAÇÃO:</span>
                      ...
                      {tx.documents?.length > 0 && <DocList docs={tx.documents} />}
