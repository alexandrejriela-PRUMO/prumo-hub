...
import {
  Scale, Plus, Calendar, FileText, Users, Clock,
  AlertCircle, CheckCircle, Pause, Archive, Trash2,
  MapPin, DollarSign, ChevronLeft, Radio, RefreshCw
} from 'lucide-react';
...
function ProcessCard({ process, canCreateProcess, onEdit, onDelete, onShowHistory }) {
...
// ── ProcessForm — componente próprio fora do pai ─────────────────────────────
function ProcessForm({ formData, setFormData, onSubmit, onCancel, editingProcess, properties }) {
  return (
...
export default function Processes() {
  ...
  const handleShowHistory = (process) => {
  ...
  const filteredProcesses = (type) => processes.filter((p) => p.process_type === type);
